...
const resolvers = {
 Query: {
    ...
    GetBlog: (_,{blogId})=>GetBlog(blogId),
    GetBlogsByUser:(_,{username})=>GetBlogsByUser(username)
 },
...
async function GetBlogsByUser(user){
    try{
        ...
        db = client.db(dbName);
        const collection =  db.collection('blogs');
        const result = await collection.find({username:user}).toArray();
        console.log('Result of find:\n', result);
        return result;
    }
    catch(err){
...
async function GetBlog(Id){
    try{
        ...
        console.log("Id passed to API is", Id);
        console.log('Result of find from get blog:\n', result);
        return result;
    }
    catch(err){
        console.log(err);
...
async function AddComment(_,{comment}){
    try{
        ...
        const cmt = {blogId: comment.blogId,comment: comment.comment,createdDate: new Date(), username: comment.username};
